...
class _BusUpto6FormScreenState extends State<BusUpto6FormScreen> {
  ...
  void _submitForm() {
    ...
    double geographicalExtnAmount = double.tryParse(_selectedGeographicalExtn ?? '0') ?? 0.0;
    ...
    double premiumBeforeCess = netOdPremium + tpPremium + paOwnerDriver + llPaidDriverAmount + geographicalExtnAmount;
    ...
    Map<String, String> resultData = {
      ...
      'Restricted TPPD': restrictedTppdYes ? 'Yes (10% discount applied)' : 'No',
                  ...
                  child: Form(
                    ...
                    child: Column(
                      children: [
                        ...
                        _buildTextField('accessoriesValue', 'Electrical/Electronic Accessories (₹)', 'Enter Value'),
                        _buildDropdownField('CNG/LPG Kits', _cngKitOptions, _selectedCngKit,
                            (val) => setState(() => _selectedCngKit = val), hintText: 'Select Option'),
                        _buildTextField('externalCngLpgKit', 'CNG/LPG Kits (Externally Fitted) (₹)', 'Enter Value'),
                        _buildDropdownField('No Claim Bonus (%)', _ncbOptions, _selectedNcb,
                            (val) => setState(() => _selectedNcb = val), hintText: 'Select Option'),
                        _buildDropdownField('Geographical Extn.', _geographicalExtnOptions, _selectedGeographicalExtn,
                            (val) => setState(() => _selectedGeographicalExtn = val), hintText: 'Select Option'),
                        ...
                        _buildDropdownField('Restricted TPPD', _restrictedTppdOptions, _selectedRestrictedTppd,
                            (val) => setState(() => _selectedRestrictedTppd = val), hintText: 'Select Option'),
...
double _getBusTpRate({required int passengerCount, bool usePerPassenger = false}) {
  if (passengerCount <= 6) {
