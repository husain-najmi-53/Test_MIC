...
class PCFormComplete extends StatefulWidget {
  const PCFormComplete({
    super.key,
  });

  @override
  State<PCFormComplete> createState() => _PCFormCompleteState();
}

class _PCFormCompleteState extends State<PCFormComplete> {
  ...
  final List<String> _voluntaryDeductOptions = ['0', '2500','5000','7500','15000',];
  ...
  void _submitForm() {
    if (_formKey.currentState!.validate()) {
      ...
      int cubicCapacity = int.tryParse(_controllers['cubicCapacity']!.text) ??
          0;
      ...
      double geographicalExt = double.tryParse(
          _controllers['geographicalExt']!.text) ?? 0.0; //
      double fiberGlassTank = double.tryParse(
          _controllers['fiberGlassTank']!.text) ?? 0.0; //
      double drivingTutions = double.tryParse(
          _controllers['drivingTutions']!.text) ?? 0.0; //
      ...
      double RSAaddons = double.tryParse(_controllers['RSAaddons']!.text) ??
          0.0;
      double consumables = double.tryParse(_controllers['consumables']!.text) ??
          0.0;
      double tyreCover = double.tryParse(_controllers['tyreCover']!.text) ??
          0.0;
      double NCBprotection = double.tryParse(
          _controllers['NCBprotection']!.text) ?? 0.0;
      double engineProtector = double.tryParse(
          _controllers['engineProtector']!.text) ?? 0.0;
      double returnToInvoice = double.tryParse(
          _controllers['returnToInvoice']!.text) ?? 0.0;
      ...
      print(basicOD);
      double antiTheftValue = _selectedAntiTheft != true ? double.parse(
          (min(basicOD * 0.025, 500)).toStringAsFixed(2)) : 0.0;
      print(antiTheftValue);
      double handicapValue = _selectedHandicap != true ? basicOD * 0.5 : 0.0; //
      print(handicapValue);
      double AAIValue = _selectedAAI != true ? double.parse(
          (min(basicOD * 0.05, 200)).toStringAsFixed(2)) : 0.0;
      print(AAIValue);
      double VoluntaryDeduct = double.tryParse(
          _selectedVoluntaryDeduct ?? "0") ?? 0.0; //
      ...
      double OptionalExtensions = geographicalExt + fiberGlassTank +
          drivingTutions;
      double TotalDiscounts = antiTheftValue + handicapValue + AAIValue +
          VoluntaryDeduct;
      double totalBasicPremium = (basicOdAfterDiscount + accessoriesValue +
          CNG_LPG_kits_Ex_fitted + OptionalExtensions) - TotalDiscounts;
      ...
      Map<String, String> resultMap = {
        ...
        "Optional Extensions(GeoExt+FiberGT+DrTutions)": OptionalExtensions
            .toStringAsFixed(2), //
        "Total Discounts(AntiTheft+Handicap+AAI+VoluntaryDeduct)": TotalDiscounts
            .toStringAsFixed(2), //
      ...
      InsuranceResultData resultData = InsuranceResultData(
        vehicleType: "Four Wheeler Complete",
        fieldData: resultMap,
          ...
          child: SingleChildScrollView(
            child: Column(
              children: [
                ...
                _buildDropdownField('Anti Theft', _antiTheftOptions, _selectedAntiTheft,
                        (val) => setState(() => _selectedAntiTheft = val)),
                _buildDropdownField('Handicap', _handicapOptions, _selectedHandicap,
                        (val) => setState(() => _selectedHandicap = val)),
                _buildDropdownField('AAI', _AAIOptions, _selectedAAI,
                        (val) => setState(() => _selectedAAI = val)),
                _buildDropdownField('Voluntary Deductible', _voluntaryDeductOptions, _selectedVoluntaryDeduct,
                ...
                _buildTextField('RSAaddons',
                    'RSA/Additional for Addons(amount)', true, "Enter 1 Yr Amount "),
                _buildTextField(
                    'consumables', 'Consumables(rate)', true, "Enter 1 Yr rate "),
                _buildTextField(
                    'tyreCover', 'Tyre Cover(rate)', true, "Enter 1 Yr rate "),
                _buildTextField('NCBprotection', 'NCB Protection(rate)', true,
                    "Enter 1 Yr rate "),
                _buildTextField(
                    'engineProtector', 'Engine Protector', true, "Enter 1 Yr Rate "),
