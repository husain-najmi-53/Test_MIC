...
class _TrailerAndOtherFormScreenState
    extends State<TrailerAndOtherFormScreen> {
  ...
  final Map<String, TextEditingController> _controllers = {
    ...
    'idv (Attached trailer)': TextEditingController(),
  ...
  final List<String> _llEmployeeOtherOptions = ['0', '50', '100', '100', '150', '200', '250', '300', '350'];
...
Widget _buildReadOnlyField(String key, String label) {
 ...
 void _submitForm() {
    ...
    double imt23 = double.tryParse(_selectedImt23 ?? "0") ?? 0.0;
    String selectedTrailerTowedBy = _selectedTrailerTowedBy ?? 'Other Vehicle';
    double overturningForCranes = double.tryParse(_selectedOverturningForCranes ?? "0") ??0.0 ;
    double discountRate = double.tryParse(_controllers['discountOnOd']!.text) ?? 0.0;
    ...
    double loadingRate = double.tryParse(_controllers['loading_on_discount_premium']!.text) ?? 0.0;
    ...
    double noOfTrailersAttached = double.tryParse(_controllers['No_of_trailers(Attached)']!.text) ?? 0.0;
    // String ncbRate = _selectedNcb ?? "0%";
    double ncbRate =double.tryParse(_selectedNcb ?? "0") ?? 0.0;
    double cngLpgKitValue = double.tryParse(_controllers['CNG/LPG_kit(Externally_fitted)']!.text) ?? 0.0;
    // double ncbPercentage = double.tryParse(ncbRate) ?? 0.0;
    ...
    // OD Calculations
    double trailerODRate = 1.0;
    double cngLpgRate = 4.0;
    double basicForVehicle = currentIdv * vehicleBasicRate / 100;
    double trailerOD = currentIdv * trailerODRate / 100;
    double basicODPremium = basicForVehicle + trailerOD;
    double overturningCranePremium = basicODPremium * overturningForCranes / 100;
    double imt23Loading = basicODPremium * imt23 / 100;
    double cngLpgPremium = cngLpgKitValue * cngLpgRate / 100;
    double basicODBeforePremium = basicODPremium + overturningCranePremium + imt23Loading + cngLpgPremium;
    double discountOnODPremium = basicODBeforePremium * discountRate / 100;
    double loadingOnODPremium = basicODBeforePremium * loadingRate / 100;
    double basicODBeforeNCB = basicODBeforePremium - discountOnODPremium + loadingOnODPremium;
    double ncbAmount = basicODBeforeNCB * ncbRate / 100;
    double netOwnDamage = basicODBeforeNCB - ncbAmount;
    double totalA = netOwnDamage;


    // TP Section
    double liabilityPremiumTP = _getTpRate(selectedTrailerTowedBy ?? 'Other Vehicle');
    ...
    Map<String, String> resultMap = {
      ...
      "Trailer IDV (Attached trailer)": (idv * 0.1).toStringAsFixed(2),
            ...
            child: Column(
              ...
              _buildDropdownField(
                ...
                _buildDropdownField(
                    ...
                    (val) => setState(() => _selectedDepreciationAttached = val),
                ),
              _buildReadOnlyField(
                    'currentIdv (Attached)', 'Current IDV (Attached) (₹)'), // Read-only field
              _buildTextField('No_of_trailers(Attached)', 'No. of trailers(Attached)','No_of_trailers(Attached)'),
              ...
              _buildDropdownField('Overturning For Cranes', _overturningForCranesOptions, _selectedOverturningForCranes,
              ...
              _buildTextField('Year_of_manufacture', 'Year of manufacture','Year of manufacture'),
              _buildDropdownField('Zone', _zoneOptions, _selectedZone,
                    (val) => setState(() => _selectedZone = val)),
              _buildTextField('Discount_on_OD_premium(%)', 'Discount on OD premium(%)','Discount on OD premium(%)'),
              ...
              _buildTextField('CNG/LPG_kit(Externally_fitted)', 'CNG/LPG kit(Externally fitted)','CNG/LPG kit(Externally fitted)'),
              ...
              _buildTextField('PA_to_owner_driver', 'PA to owner driver','PA to owner driver'),
              ...
              _buildTextField('Other_CESS(%)', 'Other CESS(%)','Other CESS(%)'),
...
double _getTpRate (String trailerTowedBy) {
  ...
  if (trailerTowedBy == 'Agriculture Tractors upto 6 HP') {
    ...
    return 0; // fallback/default TP premium if type unknown
  }
}
